...
/// Überprüfen, ob ein Monat ausgewählt wurde, oder auf den aktuellen Monat zurückgreifen
$selectedMonth = isset($_GET['selected_month']) ? $_GET['selected_month'] : date("Y-m");
$currentMonthText = strftime("%B %Y", strtotime($selectedMonth . "-01")); // Monat und Jahr in Deutsch
...
// Monatsdaten abrufen
$sqlTotal = "SELECT SUM(Gesamtbetrag) AS gesamtbetrag, SUM(drinkssumme) AS drinkssumme, COUNT(*) AS anzahl FROM Bestellungentest WHERE DATE(Bestelldatum) BETWEEN ? AND ?";
$stmtTotal = $conn->prepare($sqlTotal);
$stmtTotal->bind_param("ss", $startOfMonth, $endOfMonth);
$stmtTotal->execute();
...
$totalDrinkSum = $dataTotal['drinkssumme'] ?? 0;  // Speichert die drinkssumme des Monats
$totalOrders = $dataTotal['anzahl'] ?? 0;

// Summe Abholen und Lieferung berechnen
$sqlOrderTypes = "SELECT bestell_typ, SUM(Gesamtbetrag) AS gesamtbetrag, SUM(drinkssumme) AS drinkssumme, COUNT(*) AS anzahl FROM Bestellungentest WHERE DATE(Bestelldatum) BETWEEN ? AND ? GROUP BY bestell_typ";
$stmtOrderTypes = $conn->prepare($sqlOrderTypes);
$stmtOrderTypes->bind_param("ss", $startOfMonth, $endOfMonth); // Korrektur hier
...
$sqlStornos = "SELECT COUNT(*) AS anzahl, SUM(Gesamtbetrag) AS gesamtbetrag, SUM(drinkssumme) AS drinkssumme FROM Bestellungentest WHERE DATE(Bestelldatum) BETWEEN ? AND ? AND Freigegeben = 4";
$stmtStornos = $conn->prepare($sqlStornos);
$stmtStornos->bind_param("ss", $startOfMonth, $endOfMonth);
$stmtStornos->execute();
...
$sqlDeliveryCosts = "SELECT SUM(Lieferkosten) AS lieferkosten FROM Bestellungentest WHERE DATE(Bestelldatum) BETWEEN ? AND ? AND bestell_typ = 'Liefern'";
$stmtDeliveryCosts = $conn->prepare($sqlDeliveryCosts);
$stmtDeliveryCosts->bind_param("ss", $startOfMonth, $endOfMonth);
...
$totalUebersicht = ($totalVorLiefern + $lieferkostenTotal) - $stornoTotal;

// Zahlungsarten
$sqlPayments = "SELECT Zahlungsmethode, SUM(Gesamtbetrag) AS gesamtbetrag, SUM(drinkssumme) AS drinkssumme, COUNT(*) AS anzahl FROM Bestellungentest WHERE Freigegeben !=4 AND DATE(Bestelldatum) BETWEEN ? AND ? GROUP BY Zahlungsmethode";
$stmtPayments = $conn->prepare($sqlPayments);
$stmtPayments->bind_param("ss", $startOfMonth, $endOfMonth); // Korrektur hier
...
$tax19AmountBar = $barTotalDrink / (1 + $taxRate19 / 100) * ($taxRate19 / 100);

$barNetto = $barTotal - $tax7AmountBar -$tax19AmountBar;

$tax7AmountEC = $ecTotal / (1 + $taxRate7 / 100) * ($taxRate7 / 100);
$tax19AmountEC = $ecTotalDrink / (1 + $taxRate19 / 100) * ($taxRate19 / 100);
...
.totals-table td, .payment-table td, .tax-table td {
    padding: 4px 5px; /* Kleinere Polsterung */
...
<div class="header">
    ...
    <p>Tel: 08381/7478</p>
    <p><strong>Monat: <?php echo $currentMonthText; ?></strong></p>
    <hr>
</div>
...
<div class="footer">
    <p>Erzeugt am <?php echo date("d.m.Y H:i"); // Zeigt das aktuelle Datum und die aktuelle Uhrzeit ?></p>
</div>
